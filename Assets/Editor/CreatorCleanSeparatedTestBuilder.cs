...
public static class CreatorCleanSeparatedTestBuilder
{
    private const string FolderPath = "Assets/Resource/Model/Weapon/CreatorCleanSeparated";
    private const string ArmsFbxPath = FolderPath + "/CreatorClean_Arms.fbx";
    private const string GunFbxPath = FolderPath + "/CreatorClean_Gun.fbx";
    private const string ArmsControllerPath = FolderPath + "/CreatorClean_Arms.controller";
    private const string GunControllerPath = FolderPath + "/CreatorClean_Gun.controller";
    private const string ScenePath = "Assets/Scenes/Test/CreatorCleanSeparated_TestScene.unity";
    private const string MarkerPath = "Library/CreatorCleanSeparatedTestBuilder.done";
    ...
    [MenuItem("Tools/Test/Build Creator Clean Separated Test")]
    public static void Build()
    {
        Directory.CreateDirectory(FolderPath);
        Directory.CreateDirectory("Assets/Scenes/Test");

        var armsClipFbxPaths = Directory.GetFiles(FolderPath, "CreatorClean_Arms*.fbx")
            .Select(NormalizeAssetPath)
            .ToArray();
        var gunClipFbxPaths = Directory.GetFiles(FolderPath, "CreatorClean_Gun*.fbx")
            .Select(NormalizeAssetPath)
            .ToArray();

        foreach (var fbxPath in armsClipFbxPaths.Concat(gunClipFbxPaths))
        {
            ConfigureModelImporter(fbxPath);
            AssetDatabase.ImportAsset(fbxPath, ImportAssetOptions.ForceUpdate);
        }

        var armsController = BuildController(ArmsControllerPath, armsClipFbxPaths, "FP_", new[]
        {
            new ClipBinding("Idle", "idle_pose"),
            new ClipBinding("Fire", "fire"),
            new ClipBinding("Reload", "reload"),
            new ClipBinding("RefPose", "ref_pose"),
        });

        var gunController = BuildController(GunControllerPath, gunClipFbxPaths, "GUN_", new[]
        {
            new ClipBinding("Idle", "idle"),
            new ClipBinding("Fire", "fire"),
            new ClipBinding("Reload", "reload"),
            new ClipBinding("Walk", "walk"),
            new ClipBinding("RefPose", "ref_pose"),
        });
        ...
        Debug.Log("Creator clean separated test scene built.");
    }
    ...
    private static AnimatorController BuildController(string controllerPath, string[] fbxPaths, string clipPrefix, ClipBinding[] bindings)
    {
        var clips = fbxPaths
        ...
        if (clips.Length == 0)
        {
            throw new InvalidOperationException($"No animation clips found: {string.Join(", ", fbxPaths)}");
        }
        ...
        Debug.Log($"{Path.GetFileName(controllerPath)} clips: {string.Join(", ", clips.Select(clip => clip.name))}");
        return controller;
    }
    ...
    private static void BuildScene(RuntimeAnimatorController armsController, RuntimeAnimatorController gunController)
    {
        ...
        scene.name = "CreatorCleanSeparated_TestScene";

        var root = new GameObject("CreatorCleanSeparatedPreviewRoot");
        var arms = InstantiateModel(ArmsFbxPath, "CreatorClean_Arms", armsController, root.transform);
        var gun = InstantiateModel(GunFbxPath, "CreatorClean_Gun", gunController, root.transform);

        var player = root.AddComponent<CreatorCleanSeparatedKeyboardAnimationPlayer>();
